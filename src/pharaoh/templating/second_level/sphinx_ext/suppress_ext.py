...
logger associated with the named extension (e.g., ``ext.intersphinx`` to
...
def _parse_levels(levels):
    if not isinstance(levels, (list, tuple)) and isinstance(levels, (int, str)):
        levels = [levels]
...
class SphinxSuppressLogger(SphinxSuppressFilter):
    ...
    def suppressed(self, record):
        return logging.Filter.filter(self, record) and (self.levels is ALL or record.levelno in self.levels)
...
class SphinxSuppressRecord(SphinxSuppressLogger, SphinxSuppressPatterns):
    ...
    def suppressed(self, record):
        log_sup = SphinxSuppressLogger.suppressed(self, record)
        pat_sup = SphinxSuppressPatterns.suppressed(self, record)
        return log_sup and pat_sup
...
def _get_filters(config):
    ...
    def is_pattern(s):
        return isinstance(s, (str, re.Pattern))
